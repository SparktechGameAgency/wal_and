...
////    public void OpenArmyPanel() => SetActivePanel(GameState.Army);
////    public void OpenCastlePanel() => SetActivePanel(GameState.Castle);
////    public void OpenVillagePanel() => SetActivePanel(GameState.Village);
...
//    /// <summary>
//    /// Opens the Castle Panel and immediately activates the Archer tab.
//    /// Called after buying a soldier from the ArcherZone context.
//    /// </summary>
//    public void OpenCastlePanelWithArcherTab()
//    {
//        SetActivePanel(GameState.Castle);
//        if (CastleGridMover.Instance != null)
//            CastleGridMover.Instance.OpenCastlePanel();
//        // Activate the Archer tab so zones & slots reflect the right state.
//        CastleTabController.Instance?.ActivateTab(CastleTabController.CastleTab.Archer);
//    }
...
//            case GameState.Village:
//                SetPanelVisible(villagePanel, true);
//                // Hide ALL interactive zones — village shows the updated castle
//                // visually but never cannon/archer drop zones.
//                CastleUnitDropZone.SetCannonZonesVisible(false);
//                ArcherZoneCastle.SetArcherZonesVisible(false);
//                // Also collapse every slot's zone overlays via the tab controller.
//                foreach (var slot in UnityEngine.Object.FindObjectsByType<CastleBlockUnitSlot>(
//                             UnityEngine.FindObjectsInactive.Include,
//                             UnityEngine.FindObjectsSortMode.None))
//                    slot.SetVillageMode(true);
//                break;
//            case GameState.Army:
//                SetPanelVisible(armyPanel, true);
//                CastleUnitDropZone.SetCannonZonesVisible(false);
...
public class GameManager : MonoBehaviour
{
    ...
    public void SpawnBasicSoldier()
    {
        ...
        // Always spawn soldiers as children of SoldierSpawnArea so they
        // automatically move with it when it reparents between panels.
        Transform spawnParent = CastleGridMover.Instance != null && CastleGridMover.Instance.soldierSpawnArea != null
            ? CastleGridMover.Instance.soldierSpawnArea
            : (villageSpawnPoint != null ? villageSpawnPoint.parent ?? villageSpawnPoint : null);

        if (spawnParent == null) { Debug.LogError("[GameManager] No valid spawn parent found!"); return; }

        GameObject soldier = Instantiate(basicSoldierPrefab, spawnParent.position, Quaternion.identity, spawnParent);
        OnSoldierSpawned?.Invoke(soldier);
    }
