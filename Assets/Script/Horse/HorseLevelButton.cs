...
/// HorseLevelButton
///
/// Buy mode cards  → show horse NAME  (e.g. "Brown Horse")
/// Inventory cards → show horse NAME + (N/total) badge
///
/// The panel's upper-centre bar (horseLevelText in HorsePanelManager) is the
/// only place that shows the numeric level — cards never show "Level X" anymore.
/// </summary>
public class HorseLevelButton : MonoBehaviour
{
    [Header("Wire from Hierarchy")]
    ...
    /// <summary>
    /// Buy-mode card. Shows the horse NAME (not level) so the player can
    /// identify horses by name. The level is shown in the panel's top bar.
    /// </summary>
    public void Setup(HorseData data, HorsePanelManager manager, bool locked)
    {
        _data = data;
        ...
        // ── CHANGE: show horse NAME, not "Level X"
        if (levelText != null)
        {
            levelText.text = data.horseName;
            levelText.fontSize = 26;
    ...
    /// Inventory-mode card.
    ///
    /// sellIndex  – position in the owned list (unique per card, even duplicates).
    /// typeIndex  – 1-based position among same-type horses  (e.g. 1, 2 …).
    /// typeTotal  – total copies of this horse type owned    (e.g. 2).
    ///
    /// Card label example: "Brown Horse (1/2)"
    /// </summary>
    public void SetupForInventory(HorseData data, HorsePanelManager manager,
                                  int sellIndex, int typeIndex, int typeTotal)
    {
