...
///
/// KEY FIXES:
///   • Live stats (health, ability, damage) are stored HERE per-slot,
///     NOT on HorseData (which is a shared ScriptableObject asset).
///     This means upgrading Horse A never affects Horse B.
///
///   • Upgrade timer runs in Update() independently.  The panel can be
///     closed, another horse selected, or anything else — the timer
///     still counts down and applies the stat gains when it expires.
///
///   • UpgradeCount / IsUpgrading / UpgradeTimeRemaining are exposed
///     so HorsePanelManager can read live state from whichever slot
