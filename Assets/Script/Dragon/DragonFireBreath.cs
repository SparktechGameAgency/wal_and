...
public class DragonFireBreath : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Auto-find FirePoint if not assigned in Inspector.
        // Search order:
        //   1. DragonRider/FirePoint  (preferred — lives inside the rider child prefab)
        //   2. FirePoint              (direct child fallback)
        //   3. null → particles spawn from dragon pivot with a warning
        if (firePoint == null)
        {
            Transform found = transform.Find("DragonRider/FirePoint")
                           ?? transform.Find("FirePoint");

            if (found != null)
            {
                firePoint = found.GetComponent<RectTransform>();
                Debug.Log($"[DragonFireBreath] Auto-found FirePoint at path '{found.name}'.", this);
            }
            else
            {
                Debug.LogWarning("[DragonFireBreath] No FirePoint found at 'DragonRider/FirePoint' " +
                                 "or 'FirePoint'. Fire will spawn from the dragon's pivot. " +
                                 "Add an empty child named 'FirePoint' inside the DragonRider GameObject.", this);
            }
        }
    ...
    /// <summary>
    /// Forces this component to re-resolve its Canvas and rebuild its pooled
    /// particle container from scratch. Call this explicitly right after a
    /// carried-over dragon (DontDestroyOnLoad'd via its FlyZone — see
    /// BattleStarter.CarryDragonsIntoBattle) is reparented into the Battle
    /// scene's Canvas — see BattleManager.ReceivePlayerDragons.
    ///
    /// Awake() only ever runs once, back in the Village scene: it caches
    /// rootCanvas/_canvasRt AND builds _poolParent as a child of THAT
    /// canvas. _poolParent is a SIBLING of the dragon, not a child of it,
    /// so DontDestroyOnLoad (which only protects the FlyZone/dragon
    /// GameObject) never carries it over — it, and every pooled particle
    /// inside it, is destroyed along with the rest of the Village scene
    /// the instant it unloads, and rootCanvas/_canvasRt end up pointing at
    /// a destroyed Canvas too.
    ///
    /// Relying on OnEnable to catch this wouldn't work even if one existed
    /// here — the GameObject is never actually disabled/re-enabled during
    /// the scene transition, it just persists — same reasoning
    /// BattleDragonFlight.ActivateForBattle() already documents for why
    /// IT can't just depend on lifecycle callbacks either. Without this,
    /// Play() silently no-ops on a destroyed _poolParent and every
    /// SpawnParticle() call throws a MissingReferenceException on the
    /// destroyed pooled RectTransforms — logged to console, invisible to
    /// the player. That's the exact "player dragon doesn't breathe fire"
    /// symptom; a freshly-Instantiated bot dragon never touches a Village
    /// canvas at all, so it never hits this path.
    /// </summary>
    public void RebindToCurrentScene()
    {
        rootCanvas = GetComponentInParent<Canvas>();
        if (rootCanvas == null)
        {
            Debug.LogError("[DragonFireBreath] No Canvas found on rebind.", this);
            return;
        }
        _canvasRt = rootCanvas.GetComponent<RectTransform>();

        // Drop every reference to the old (destroyed) pool before rebuilding —
        // otherwise _pool/_active still hold RectTransforms from the
        // Village-scene _poolParent that no longer exists.
        _pool.Clear();
        _active.Clear();
        if (_poolParent != null) Destroy(_poolParent); // no-op if already gone
    ...
    private void SpawnParticle()
    {
        ...
        if (_aimTarget != null)
        {
            // Was (Vector2)_aimTarget.position — the raw RectTransform
            // pivot, which sits at the target's feet for ground units, so
            // fire was consistently hitting the "down side" of the enemy
            // instead of its body. See AimPointUtil for the full
            // explanation. Read live every particle so the flame keeps
            // tracking a moving target, same as before.
            Vector2 aimPoint = AimPointUtil.GetBodyCenter(_aimTarget);
            Vector2 toTarget = aimPoint - (Vector2)origin.position;
