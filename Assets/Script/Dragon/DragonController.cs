    ...
    IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    [Header("Combat — Approach Offset")]
    [Tooltip("Canvas-unit offset applied to the enemy's position when chasing. " +
             "X: positive = stop to the right of the enemy, negative = to the left. " +
             "Y: positive = hover above, negative = below. " +
             "Tip: use a negative Y (e.g. -80) to stop beside the enemy instead of on top.")]
    [SerializeField] private Vector2 approachOffset = new Vector2(0f, -80f);

    [Header("Combat — Fire Burst Cycle")]
    [Tooltip("How many seconds the dragon breathes fire each burst.")]
    [SerializeField] private float fireDuration = 1.5f;
    [Tooltip("How many seconds the dragon rests between bursts (window for enemies to attack).")]
    ...
    // Burst/cooldown cycle: dragon fires for fireDuration, rests for fireCooldown, repeat.
    private enum FireCycleState { Idle, Firing, Resting }
    private FireCycleState _fireCycle = FireCycleState.Idle;
    private float _fireCycleTimer;
    ...
    /// <summary>
    /// The dragon's rider seat, or null if this dragon has no seat wired up.
    /// Used by BattleStarter to check occupancy and snapshot the rider's
    /// equipment for the Battle scene.
    /// </summary>
    public DragonRiderSeat RiderSeat => _riderSeat;
    ...
    private void DoCombatAI()
    {
        ...
        // Use the shared helper so DoCombatAI and ChaseTarget always agree on
        // where the dragon's hover point is. Changing approachOffset in the
        // Inspector immediately moves that point for both systems.
        Vector2 approachAP = GetApproachPosition(_target);

        // Attack when the dragon has reached the approach point (within a small
        // tolerance), not when it is within attackRange of the raw enemy position.
        // This means approachOffset fully controls where the dragon hovers.
        float arrivalThreshold = 8f; // canvas units — close enough to start breathing fire
        float distToApproach = Vector2.Distance(_rt.anchoredPosition, approachAP);

        if (distToApproach <= arrivalThreshold)
        {
            // Dragon has arrived — run the burst/rest cycle instead of firing continuously.
            FaceDirection(_target.transform.position.x - transform.position.x);
            TickFireCycle();
        }
        else
        {
            // Still chasing — reset cycle so it starts fresh on arrival.
            if (_isAttacking) StopAttacking();
            _fireCycle = FireCycleState.Idle;
            ChaseTarget(_target);
        }
    }
    ...
    /// </summary>
    /// <summary>
    /// Returns the canvas anchoredPosition the dragon should fly toward when
    /// chasing <paramref name="target"/> — the enemy's canvas position shifted
    /// by <see cref="approachOffset"/> so the dragon stops beside the enemy
    /// rather than directly on top of it.
    /// </summary>
    private Vector2 GetApproachPosition(EnemyUnit target)
    {
        Vector2 targetAP = WorldToCanvasAnchoredPos(target.transform.position);
        // Flip X offset based on which side we're approaching from so a positive
        // X value always means "stay back from the enemy" regardless of direction.
        float facingSign = (targetAP.x >= _rt.anchoredPosition.x) ? 1f : -1f;
        return targetAP + new Vector2(approachOffset.x * facingSign, approachOffset.y);
    }
    ...
    private void ChaseTarget(EnemyUnit target)
    {
        ...
        Vector2 targetAP = WorldToCanvasAnchoredPos(target.transform.position);
        Vector2 approachAP = GetApproachPosition(target);
        Vector2 myAP = _rt.anchoredPosition;

        Vector2 newAP = Vector2.MoveTowards(myAP, approachAP, speed * Time.deltaTime);
    ...
    /// <summary>
    /// Burst/rest fire cycle — called every frame while the dragon is in attack range.
    /// Fires for <see cref="fireDuration"/> seconds, rests for <see cref="fireCooldown"/>
    /// seconds, then repeats. The rest window is when enemies can counter-attack.
    /// </summary>
    private void TickFireCycle()
    {
        switch (_fireCycle)
        {
            case FireCycleState.Idle:
                // First arrival — kick off the firing phase immediately.
                _fireCycle = FireCycleState.Firing;
                _fireCycleTimer = fireDuration;
                StartAttacking();
                break;

            case FireCycleState.Firing:
                // Update aim so fire tracks a moving enemy.
                fireBreath?.SetTarget(_target?.transform);
                Breathe();

                _fireCycleTimer -= Time.deltaTime;
                if (_fireCycleTimer <= 0f)
                {
                    // Burst finished — enter rest phase.
                    StopAttacking();
                    _fireCycle = FireCycleState.Resting;
                    _fireCycleTimer = fireCooldown;
                    Debug.Log("[DragonController] Fire burst done — resting.");
                }
                break;

            case FireCycleState.Resting:
                _fireCycleTimer -= Time.deltaTime;
                if (_fireCycleTimer <= 0f)
                {
                    // Rest over — start the next burst.
                    _fireCycle = FireCycleState.Firing;
                    _fireCycleTimer = fireDuration;
                    StartAttacking();
                    Debug.Log("[DragonController] Rest done — firing again.");
                }
                break;
        }
    }
