...
public class CharacterEquipment : MonoBehaviour
{
    ...
    private void Start()
    {
        // ── Fix: skip default loadout if custom items were already equipped ──
        // ApplySelectionToSoldier() calls Equip() which sets _customLoadoutApplied.
        // That call happens between Instantiate() and the first frame's Start(),
        // so by the time we reach here the flag is already true for spawned soldiers.
        if (_customLoadoutApplied)
        {
            Debug.Log("[CharacterEquipment] Custom loadout already applied — skipping default loadout.");
            return;
        }
    ...
    public void Equip(EquipmentItem item)
    {
        ...
        // Mark that at least one custom item has been equipped.
        // This prevents Start() from overwriting with the default loadout.
        _customLoadoutApplied = true;
